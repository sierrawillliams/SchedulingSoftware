...
// Client pill colours (purple-coordinated palette)
const PILL_STYLES = [
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const CSS = `
@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&family=Montserrat:wght@600;700&display=swap');
...
html,body{color-scheme:light;}body{font-family:'DM Sans',sans-serif;background:var(–bg);color:var(–text);min-height:100vh;}
...
background:linear-gradient(160deg,var(–purple-dark) 0%,var(–purple) 55%,#7c4fc0 100%);
...
box-shadow:0 40px 80px rgba(0,0,0,.3);position:relative;z-index:1;animation:fadeUp .4s ease;}
...
.login-brand-icon{width:52px;height:52px;background:linear-gradient(135deg,var(–purple),var(–purple-mid));
border-radius:14px;display:flex;align-items:center;justify-content:center;
font-family:'Montserrat',sans-serif;font-size:1.1rem;font-weight:700;color:black;
box-shadow:0 6px 16px rgba(74,45,143,.35);}
.login-brand-text{font-family:'Montserrat',sans-serif;font-size:1rem;font-weight:700;color:var(–purple);line-height:1.2;}
.login-brand-text small{font-family:'DM Sans',sans-serif;font-size:.72rem;font-weight:500;color:var(–muted);display:block;}
...
.field input,.field select{width:100%;padding:.7rem 1rem;border:1.5px solid var(–border);border-radius:10px;
...
.btn-primary{background:linear-gradient(135deg,var(–purple),var(–purple-mid));color:black;
...
.btn-icon{padding:.4rem;width:30px;height:30px;justify-content:center;border-radius:7px;}
...
.sidebar-brand-name{font-family:'Montserrat',sans-serif;font-size:.9rem;font-weight:700;color:black;line-height:1.2;}
.sidebar-brand-name small{font-family:'DM Sans',sans-serif;font-size:.65rem;font-weight:400;color:#3d1f6e;display:block;}
...
.nav-item{display:flex;align-items:center;gap:.65rem;padding:.6rem 1.25rem;color:rgba(0,0,0,1);
font-size:.84rem;font-weight:500;cursor:pointer;transition:all .15s;border-left:3px solid transparent;margin:.05rem 0;}
.nav-item:hover{background:rgba(255,255,255,.08);color:black;}
.nav-item.active{background:rgba(255,255,255,.12);color:#3d1f6e;border-left-color:var(–gold);}
...
font-size:.78rem;font-weight:700;color:black;background:linear-gradient(135deg,var(–gold),var(–gold-light));
color:var(–purple-dark);}
.avatar.purple{background:linear-gradient(135deg,var(–purple-mid),var(–purple-light));color:black;}
.user-chip-name{font-size:.8rem;font-weight:600;color:black;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.user-chip-role{font-size:.67rem;color:rgba(61,31,110,1);text-transform:capitalize;}
...
.stat-label{font-size:.7rem;font-weight:500;color:var(–muted);}
.stat-warn{font-size:.68rem;color:var(–red);font-weight:600;}
...
.grid-head{background:linear-gradient(135deg,var(–purple-dark),var(–purple));
padding:.65rem .5rem;text-align:center;}
.grid-head-staff{background:linear-gradient(135deg,var(–purple-dark),var(–purple));
...
.grid-head-day{font-size:.74rem;font-weight:700;color:black;text-transform:uppercase;letter-spacing:.05em;}
...
.grid-head.is-today .grid-head-day,.grid-head.is-today .grid-head-date{color:black;}
...
.row-staff-hrs{font-size:.67rem;color:var(–muted);}
...
padding:.22rem;color:var(–muted);font-size:.68rem;cursor:pointer;
...
.avail-dot-label{font-size:.6rem;font-weight:700;color:var(–muted);}
...
.avail-dot.off .avail-dot-val{color:var(–muted);}
.overtime-badge{display:inline-block;margin-top:.5rem;font-size:.68rem;font-weight:700;
background:var(–gold-bg);color:#6b3f00;border:1px solid var(–gold-border);
padding:.15rem .5rem;border-radius:20px;}

/* ── Tags input ── */
.tags-wrap{border:1.5px solid var(–border);border-radius:10px;padding:.5rem;
display:flex;flex-wrap:wrap;gap:.3rem;background:var(–bg);min-height:42px;cursor:text;}
.tags-wrap:focus-within{border-color:var(–purple-mid);background:white;}
.tag-item{background:var(–purple-bg);color:var(–purple);font-size:.76rem;font-weight:700;
padding:.18rem .5rem;border-radius:6px;display:flex;align-items:center;gap:.3rem;}
.tag-x{cursor:pointer;opacity:.55;font-size:.8rem;line-height:1;}
...
.avail-off-btn{font-size:.62rem;padding:.2rem;border:none;background:var(–red-bg);color:var(–red);
border-radius:5px;cursor:pointer;text-align:center;}
.avail-on-btn{font-size:.62rem;padding:.25rem;border:none;background:var(–purple-bg);color:var(–purple);
border-radius:5px;cursor:pointer;text-align:center;}

/* ── Profile ── */
.profile-header{background:linear-gradient(135deg,var(–purple-dark),var(–purple));border-radius:var(–radius);
padding:1.75rem;margin-bottom:1.25rem;color:black;display:flex;align-items:center;gap:1.25rem;}
...
.hours-ring{width:80px;height:80px;border-radius:50%;display:flex;flex-direction:column;
align-items:center;justify-content:center;font-family:'Montserrat',sans-serif;}
.ring-num{font-size:1.5rem;font-weight:700;line-height:1;}
...
<p style={{fontSize:".72rem",color:"var(–muted)",marginTop:"1rem",textAlign:"center"}}>
...
function ShiftModal({ shift, users, shifts, weekDates, onSave, onDelete, onClose, isAdmin, currentUser }) {
...
shifts.filter(s => s.id !== form.id),
form.userId, weekDates
) + calcUniqueHours([form]);
...
{isNew ? "Add Shift" : "Edit Shift"}
    ...
    <div className="field">
      ...
      {selUser && !selUser.trainedClients.includes(form.client) && (
        <p style={{fontSize:".72rem",color:"var(--red)",marginTop:".25rem"}}>
          ⚠ {selUser.name.split(" ")[0]} is not trained with this client
        </p>
      )}
    </div>
    ...
    {over && (
      <div className="error-msg">⚠ This shift puts {selUser?.name.split(" ")[0]} over 40h this week ({hrs}h total). Overtime is not enabled for this staff member.</div>
    )}
    {!over && selUser && (
...
<div className="modal-title">
<span>{isNew?"👤":"✎"}</span>
{isNew ? "Add Staff Member" : `Edit — ${form.name}`}
</div>
    ...
    <p style={{fontSize:".75rem",color:"var(--muted)",marginBottom:".5rem"}}>Select all clients this staff member is trained to support</p>
    ...
    <p style={{fontSize:".75rem",color:"var(--muted)",marginBottom:".75rem"}}>Click a day to toggle availability, then set hours</p>
  ...
  <div className="profile-grid">
    <div className="pcard">
      ...
      <div className="hours-ring-wrap">
        <div className="hours-ring" style={{background: over?"var(--red-bg)":"var(--purple-bg)"}}>
          ...
          <div className="ring-label" style={{color: over?"var(--red)":"var(--muted)"}}>hrs</div>
        </div>
    ...
    <div className="pcard">
      ...
      <div className="client-tags">
        ...
        {user.trainedClients.length===0 && <span style={{fontSize:".78rem",color:"var(--muted)"}}>No clients assigned</span>}
      </div>
    </div>
    ...
    <div className="pcard" style={{gridColumn:"1/-1"}}>
      ...
      {DAYS.filter(d=>user.availability?.[d]?.length>0).map(day=>(
        <div key={day} style={{fontSize:".75rem",color:"var(--muted)",marginTop:".4rem",display:"flex",gap:".5rem",alignItems:"center"}}>
    ...
    <div className="pcard" style={{gridColumn:"1/-1"}}>
        ...
        return (
          <div key={ds} style={{marginBottom:".75rem"}}>
            <div style={{fontSize:".75rem",fontWeight:700,color:"var(--purple)",marginBottom:".35rem"}}>
              ...
              <span style={{fontWeight:400,color:"var(--muted)",marginLeft:".4rem"}}>({calcUniqueHours(dayShifts)}h)</span>
            </div>
      ...
      {shifts.filter(s=>s.userId===user.id&&weekDates.map(d=>d.toISOString().split("T")[0]).includes(s.date)).length===0 && (
        <p style={{fontSize:".8rem",color:"var(--muted)"}}>No shifts scheduled this week</p>
      )}
    </div>
  </div>
</div>
              ...
              return (
                <div key={u.id} className="stat-card">
                  <div className="stat-head">
                    ...
                    <span style={{fontSize:".76rem",fontWeight:600,color:"var(--purple)"}}>{u.name.split(" ")[0]}</span>
                  </div>
                  <div className={`stat-val ${over?"warn":""}`}>{hrs}<span style={{fontSize:".7rem",fontWeight:500,color:"var(--muted)"}}>h</span></div>
                  ...
                  if (di===0) return [
                    <div key={`lbl-${u.id}`} className="row-label">
                      <div className="row-label-inner">
                        ...
                        <div>
                          <div className="row-staff-name">{u.name.split(" ")[0]}</div>
        ...
        <div className="staff-grid">
          {users.map(u=>(
            <div key={u.id} className="staff-card">
              ...
              <div style={{fontSize:".72rem",color:"var(--muted)",marginBottom:".5rem"}}>@{u.username}</div>
              <div className="sc-label">Trained Clients</div>
              <div className="client-tags">
                {u.trainedClients.map(c=><span key={c} className="ctag">{c}</span>)}
                {!u.trainedClients.length&&<span style={{fontSize:".72rem",color:"var(--muted)"}}>None assigned</span>}
              </div>
